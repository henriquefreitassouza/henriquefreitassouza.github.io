...
Para ser caracterizado como um web app, o script só precisa de uma das funções. Usaremos a doGet apenas para tratar a saída caso o endereço do script seja acessado diretamente. A função que vai receber e tratar os dados do RD Station é a doPost. Ela recebe dados passados no corpo de uma mensagem que trafega pela web. Essas mensagens são lidas por servidores, e é justamente o papel de servidor que o nosso script está fazendo.
